# Final Code with Dynamic Thresholding + Image/PDF Support
...
def analyze_sheet(image: np.ndarray, debug_dir: str) -> Dict:
    ...
    for col in cols:
        ...
        for sq in col:
            ...
            row_circles = [
                c for c in circle_points
                if abs(c["y"] - sq["cy"]) < row_thresh
            ]

            sorted_circles = sorted(
                [c for c in row_circles if c["x"] < sq["cx"]],
                key=lambda c: c["x"],
                reverse=True
            )
    ...
    return OrderedDict(
        sorted(filled_answers.items(), key=lambda x: int(x[0][1:]))
    )
